...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  return starWarsArr.sort((obj1,obj2) => Number(obj2.height)-Number(obj1.height) )
}
...
const howMuchPencil = (str) => {
  ...
  for (let idx = 0; idx < str.length+1; idx++) {
    result.push(str.slice(idx,str.length)) 
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  for (let i=0; i<recipe.ingredients.length; i++){
   strArr.push(recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(' ')+1));
   result.push(strArr[i].slice(strArr[i].indexOf(' ')+1));
  }
  return result;
};
